...
{
    internal const string ErrorMessageMustBeAbsolute =
        "The Uri must be an absolute Uri even when converting to a RelativeUrl";

    public static Uri ToUri(this AbsoluteUrl url) => new(url.ToString());
    ...
    public static AbsoluteUrl ToAbsoluteUrl(this Uri uri)
    {
        ...
        var userInfoTokens = uri.UserInfo?.Split([':'], StringSplitOptions.RemoveEmptyEntries);

        var relativeUrl = ToRelativeUrl(uri);

        var userInfo =
            userInfoTokens != null && userInfoTokens.Length > 0
                ? new UserInfo(
                    userInfoTokens.First(),
    ...
    public static RelativeUrl ToRelativeUrl(this Uri uri)
    {
        ...
        var path = ImmutableList.Create(
            uri.LocalPath.Split(['/'], StringSplitOptions.RemoveEmptyEntries)
        );
        ...
        {
            queryParameterTokens = uri.Query.Substring(1).Split(['&']);
        }

        queryParametersList.AddRange(
            queryParameterTokens
                .Select(keyValueString => keyValueString.Split(['=']))
                .Select(keyAndValue => new QueryParameter(
                    keyAndValue.First(),
        ...
        return new RelativeUrl(
            path,
            queryParametersList.Count == 0 ? [] : queryParametersList.ToImmutableList(),
            fragment
        );
    }
    ...
    public static RelativeUrl ToRelativeUrl(this string relativeUrlString)
    {
        ...
        var tokens = relativeUrlString.Split(['#'], StringSplitOptions.None);

        var fragment = tokens.Length > 1 ? tokens[1] : "";

        tokens = tokens[0].Split(['?'], StringSplitOptions.None);
    ...
    public static RelativeUrl AddQueryString(
        ...
        relativeUrl == null
            ...
            : relativeUrl with
            {
                QueryParameters = relativeUrl.QueryParameters.Add(
                    new QueryParameter(fieldName, value)
                ),
            };

    public static AbsoluteUrl AddQueryParameter(
        this AbsoluteUrl absoluteUrl,
        string fieldName,
        string value
    ) =>
        absoluteUrl == null
            ...
            : absoluteUrl with
            {
                ...
                {
                    QueryParameters = absoluteUrl.RelativeUrl.QueryParameters.Add(
                        new QueryParameter(fieldName, value)
                    ),
                },
            };

    public static QueryParameter ToQueryParameter(this string fieldName, string value) =>
        new(fieldName, value);
    ...
    public static RelativeUrl WithQueryParameters<T>(this RelativeUrl relativeUrl, T item) =>
        relativeUrl == null
            ...
            : relativeUrl with
            {
                QueryParameters = typeof(T)
                    .GetProperties()
                    .Select(propertyInfo => new QueryParameter(
                        propertyInfo.Name,
    ...
    public static AbsoluteUrl WithCredentials(
        this AbsoluteUrl absoluteUrl,
        string username,
        string password
    ) =>
    ...
    public static AbsoluteUrl WithFragment(this AbsoluteUrl absoluteUrl, string fragment) =>
        absoluteUrl == null
            ...
            : absoluteUrl with
            {
                RelativeUrl = absoluteUrl.RelativeUrl with { Fragment = fragment },
            };

    public static AbsoluteUrl WithPath(
        this AbsoluteUrl absoluteUrl,
    ...
    public static ImmutableList<QueryParameter> ToQueryParameters(
        this QueryParameter queryParameter
    ) => ImmutableList.Create(queryParameter);
}
