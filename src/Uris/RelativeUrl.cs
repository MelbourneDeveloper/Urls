...
)
{
    ...
    public RelativeUrl(
        ...
        : this(
            path?.ToImmutableList() ?? ImmutableList<string>.Empty,
            query?.ToImmutableList() ?? QueryParameter.EmptyList
        ) => this.fragment = fragment ?? "";

    public RelativeUrl(
#pragma warning disable CA1054 // URI-like parameters should not be strings
        string relativeUrlString)
        : this(relativeUrlString.ToRelativeUrl())
#pragma warning restore CA1054 // URI-like parameters should not be strings
    ...
    public static RelativeUrl Empty { get; } =
        new(ImmutableList<string>.Empty, QueryParameter.EmptyList);
    #endregion
    ...
    public override string ToString() =>
        ...
        + (
            QueryParameters.Count > 0
                ? $"?{string.Join("&", QueryParameters.Select(e => e.ToString()))}"
                : ""
        )
    ...
    public override int GetHashCode() =>
        ToString().GetHashCode(StringComparison.InvariantCultureIgnoreCase);
    #endregion
}
